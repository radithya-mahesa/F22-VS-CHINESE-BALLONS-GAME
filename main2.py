...
font_restart = pygame.font.SysFont('calibri', 32, bold=True)
...
def gameover():
    ...
    restart_text = font.render('Press [ R ] to restart', True, 'aliceblue')
    screen.blit(restart_text, (235, 325))
    

def restart_game():
    global balonX, balonY, balonspeedX, balonspeedY, score, check, missileX, missileY
    for i in range(no_of_balons):
        balonX[i] = random.randint(100, 300)
        balonY[i] = random.randint(5, 10)
        balonspeedX[i] = -1
        balonspeedY[i] = 130
    score = 0
    missileX = 330
